...
public class MenuDAO {
	...
	// 메뉴 sql 쿼리
	// 메뉴 등록
	static final String menuInsert = "insert into menutable (menuId,resId,menuName,menuPrice,menuCategory,menuPic)"
		+" values ((select nvl(max(menuId),0)+1 from menutable),?,?,?,?,?)";
	// 메뉴 내용 조회
	static final String menuSelectOne = "select * from menutable where menuId=?";
	// 메뉴 리스트 조회
	static final String menuSelectAll = "select * from menutable where menuId=?";//식당id로 selectAll
	// 메뉴 수정
	static final String menuUpdate = "update menutable set menuName=?,menuPrice=?,menuCategory=?',menuPic=? where menuId=?";
	// 메뉴 삭제
	static final String menuDelete = "delete from menutable where menuId=?";
