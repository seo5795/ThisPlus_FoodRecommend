...
public class RevDAO {
	...
	// 리뷰 sql 쿼리
	// 리뷰 등록
	static final String revInsert = "insert into review (revId,memId,memName,resId,revTitle,revCont,revDate,revScore,revPic)"
			+" values ((select nvl(max(revId),0)+1 from review),?,?,?,?,?,?,?,?)";
	// 리뷰 내용 조회
	static final String revSelectOne = "select * from review where revId=?";
	// 리뷰 리스트 조회
	static final String revSelectAll = "select * from revie where revId=?";///review, 레스토랑 id를 받아와야함
	// 리뷰 수정
	...
	public boolean revInsert(RevVO vo) {
		...
		try {
			...
			pstmt.setString(6, vo.getRevRegDate());
	...
	public RevVO revSelectOne(RevVO vo) {
		...
		try {
			...
			if (rs.next()) {
				if (rs.getInt("revId")==(vo.getRevId())) {
					data = new RevVO();
					...
					data.setRevRegDate(rs.getNString("revRegDate")); // 날짜 형변환
